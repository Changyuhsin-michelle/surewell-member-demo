import { Barcode, Card, PageHeader, QRBox } from '../components/UI';
...
export default function MemberCardPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="電子會員卡" subtitle="結帳時出示 QR Code 或 Barcode。" />
      <section className="relative overflow-hidden rounded-[34px] bg-gradient-to-br from-brand-green via-[#1f9658] to-brand-deep p-5 text-white shadow-retail">
        <div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-white/10" />
        <div className="absolute -bottom-12 left-8 h-28 w-28 rounded-full bg-brand-orange/25" />
        <div className="flex items-start justify-between">
          <div>
            <p className="text-sm text-white/75">會員姓名</p>
        ...
        <div className="relative mt-5 rounded-[28px] bg-white p-4 text-slate-900 shadow-xl">
      ...
      <Card className="bg-gradient-to-br from-white to-brand-mint">
        <div className="grid grid-cols-2 gap-3">
          <div><p className="text-xs text-slate-500">會員點數</p><p className="text-xl font-black">{state.wallet.points.toLocaleString()} 點</p></div>
          <div><p className="text-xs text-slate-500">會員狀態</p><p className="text-xl font-black text-brand-green">{state.member.active ? '有效' : '停用'}</p></div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <ul className="space-y-3 text-sm">
          {['消費累積點數', '會員專屬價', '生日優惠券', '專屬活動', '儲值優惠'].map((item) => (
            <li key={item} className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3"><span className="h-2.5 w-2.5 rounded-full bg-brand-green" />{item}</li>
          ))}
        </ul>
      </Card>
    </div>
